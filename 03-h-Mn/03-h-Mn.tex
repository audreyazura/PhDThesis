...
%	We propose here to study a single Mn atom in a positively charged quantum dot.
	...
	Magnetic anisotropy depends on the local environment of magnetic atoms, and are a crucial properties for modern storage devices. We saw in Sec.~I.3.1%~\ref{MnSemiCon}
...
%	Cf Optical control of the spin of a magnetic atom in a semiconductor QD, L. Besombes et. al., Sept 2014
 ...
 that the exchange interaction between the hole and the Mn spin lifts the degeneracy of the Mn spin states. The PL of the different exciton species are then each split into six lines. For a given polarization, each line corresponds to a given Mn spin state. The ground state of a Mn-doped positively charged QD is formed by a single hole coupled to the Mn atom, leading to the same kind of splitting. Via the application of a positive bias voltage (V > 0) on the sample with a Schottky gate (see Sec.~II.3.1%~\ref{ChargedSample}
	...
	As discussed in Sec.~I.3.1%~\ref{MnSemiCon}
, in a strained self-assembled CdTe QD, a Mn atom exhibits a fine structure dominated by a weak magnetic anisotropy. Neglecting the tetrahedral crystal field of the CdTe matrix and the hyperfine coupling with the Mn nuclear spin, this fine structure is described by the effective spin Hamiltonian:
\begin{align}
\label{MnCF}
...
%	To the first order in $I_{hMn}/\Delta_{lh}$, the two perturbed ground states of the $\Lambda$ system considered here $\widetilde{|+\frac{3}{2};\Uparrow_h\rangle}$ and $\widetilde{|+\frac{5}{2};\Downarrow_h\rangle}$ can be written~\cite{CohenTannoudji}:
	As presented in Sec.~I.1.5%~\ref{VBM}
...
%	{\Large We saw the influence of strain anisotropy. We will now see a way to extract it more precisely.}\newline
	...
	{\begin{center}
		\includegraphics[width=12.5cm]{Pictures/PolarRateTotal.eps}
	\end{center}}
	\end{figure}
