...
\chapter{Coherent dynamics of Mn-doped positively charged quantum dots}
	...
	We begin presenting the energy structure of a Mn atom in a II-VI quantum dot. In the ground state, the Mn atom interact with the hole inside the QD. When an exciton is injected, the spin of the two holes cancel each other in first approximation, meaning the main interaction in the QD is given by the electron-Mn exchange interaction. This leads to optical $\Lambda$-system that we identify in the second part of the first section.
	...
	In the last section, we look at the evolution in time of the PL polarization and evidence oscillation in it, for a given $\Lambda$-level system. We show that it is caused by the anisotropy of strains in the QD plane, making positively charged exciton coupled to a single Mn atom a system really sensible to the strain state at its position.
...
%	Cf Optical control of the spin of a magnetic atom in a semiconductor QD, L. Besombes et. al., Sept 2014
		...
		Sec.~I.4 that the exchange interaction between the hole and the Mn spin lift the degeneracy of the Mn spin states. The recombination lines of the exciton states are then  each split into six. For a given polarization, each line corresponds to a give Mn spin state. The ground state of a Mn-doped positively charged quantum dot is formed by a single hole coupled to the Mn atom, leading to the same kind of splitting. This hole can be trapped via the application of a positive bias voltage on the sample with a Schottky gate (see %Sec.~\ref{ChargedSample}
		Sec.~II.2.1), a single hole is trapped in the magnetic QD and only the emission of the positively charged exciton is observed (Fig.~\ref{hMnspectra} (a)). The PL of such a QD is presented on Fig.~\ref{hMnspectra} (b).
